export const ACCOUNT_NAME = `アカウント名`;
export const ALREADY_HAVE_ACCOUNT = `アカウントをお持ちですか？`;
export const ADD_ITEM = `商品追加`;
export const ALERT_DELETE_EVENT = `以下のイベントを削除します`;
export const ALERT_DELETE_ITEM = `以下の商品を削除します`;
export const BACK = `戻る`;
export const CANCEL = `キャンセル`;
export const CHECKOUT = `お会計`;
export const CHANGE_ACCOUNT_INFO = `アカウント情報を変更`;
export const CHANGE_ACCOUNT_SUCCESS = `アカウントは更新されました`;
export const CHANGE_ACCOUNT_FATAL_ERROR = `アカウント更新に失敗しました`;
export const CONFIRM_PASSWORD = `パスワード（確認）`;
export const CONFIRM_DELETE_ACCOUNT = `アカウントを削除するにはパスワードを入力してください。`;
export const CREATE_ACCOUNT = `アカウントを作成`;
export const CREATE_ACCOUNT_FATAL_ERROR = `アカウント作成に失敗しました`;
export const CREATE_EVENT = `イベントを作成`;
export const CREATE_EVENT_FATAL_ERROR = `イベント作成に失敗しました`;
export const COUNT = `個`;
export const DELETE = `削除`;
export const DELETE_ACCOUNT = `アカウントを削除`;
export const DELETE_EVENT_FATAL_ERROR = `イベント削除に失敗しました`;
export const DELETE_EVENTITEM_FATAL_ERROR = `アイテム削除に失敗しました`;
export const DEPOSIT = `お預り金`;
export const EVENT_NAME = `イベント名`;
export const EVENTS_DASHBOARD = `イベント管理`;
export const EVENTS_LIST = `イベントリスト`;
export const GET_EVENTS_FATAL_ERROR = `イベント取得に失敗しました`;
export const ITEM_COUNT = `在庫数`;
export const ITEM_NAME = `商品名`;
export const ITEM_PRICE = `値段`;
export const ITEMS_DASHBOARD = `商品管理`;
export const LOGIN = `サインイン`;
export const LOGOUT = `サインアウト`;
export const EVENTS_URI = `/events/`;
export const MINUS = `-`;
export const NETWORK_REACH_ERROR = `サーバに接続できませんでした`;
export const NOT_HAVE_ACCOUNT = `初めてのご利用ですか？`;
export const OK = `OK`;
export const PAGE_NOT_FOUND = `お探しのページは見つかりませんでした。`;
export const PASSWORD = `パスワード`;
export const PATCH_EVENT_FATAL_ERROR = `イベント更新に失敗しました`;
export const PATCH_EVENTITEM_FATAL_ERROR = `アイテム更新に失敗しました`;
export const PAYMENT = `お支払`;
export const PLUS = `+`;
export const POST_EVENTITEM_FATAL_ERROR = `アイテム追加に失敗しました`;
export const REGI = `レジ`;
export const RELOAD_NOTIFICATION = `ページを移動・リロードすると、現在の操作は失われます。本当によろしいですか？`;
export const SAVE = `保存`;
export const SERVICE_NAME = `Regi-Urico`;
export const SUBTOTAL = `小計`;
export const THE_CHANGE = `お釣り`;
export const CONNECT_TO_TWITTER = `Twitter に接続する`;
export const YEN = `円`;
export const YEN_MARK = `¥`;

export const CONFIRM_LOGOUT = `${LOGOUT}（確認）`;
export const CONFIRM_LOGOUT_TEXT = `本当に${LOGOUT}しますか？`;
export const REGISTER_FATAL_ERROR = `${CHECKOUT}に失敗しました`;
export const SIGNIN_FATAL_ERROR = `${LOGIN}に失敗しました`;
